...
static SSL_CTX *workerSslContext = nullptr;
static SSL_CTX *getWorkerSslContext() {
    if (workerSslContext == nullptr) {
        workerSslContext = SSL_CTX_new(TLS_client_method());
        if (workerSslContext != nullptr) {
            SSL_CTX_set_mode(workerSslContext, SSL_MODE_ENABLE_FALSE_START);
        }
    }
    return workerSslContext;
}
...
void ConnectionSocket::sendWebSocketUpgradeRequest() {
    uint8_t randBytes[16];
    RAND_bytes(randBytes, 16);
    static const char *b64Chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    std::string b64Key = "";
    int val = 0, valb = -6;
    for (uint8_t c : randBytes) {
        val = (val << 8) + c;
        valb += 8;
        while (valb >= 0) {
            b64Key.push_back(b64Chars[(val >> valb) & 0x3F]);
            valb -= 6;
        }
    }
    if (valb > -6) b64Key.push_back(b64Chars[((val << 8) >> (valb + 8)) & 0x3F]);
    while (b64Key.size() % 4) b64Key.push_back('=');
    webSocketKey = b64Key;
    ...
    std::string secretParam = "";
    if (!proxySecret->empty()) {
        std::string s = *proxySecret;
        if (s.rfind("worker:", 0) == 0) {
            s = s.substr(7);
        }
        if (!s.empty()) {
            secretParam = "&secret=" + s;
        }
    }

    std::string req = "GET /?ip=" + currentAddress + "&port=" + std::to_string(currentPort) + secretParam + " HTTP/1.1\r\n"
                      "Host: " + workerHost + "\r\n"
                      "Upgrade: websocket\r\n"
                      "Connection: Upgrade\r\n"
    ...
    if (LOGS_ENABLED) DEBUG_D("connection(%p) sending Worker WS upgrade to %s target %s:%d", this, workerHost.c_str(), currentAddress.c_str(), currentPort);
    int written = SSL_write(ssl, req.data(), (int) req.size());
    if (written <= 0) {
        int err = SSL_get_error(ssl, written);
        if (err != SSL_ERROR_WANT_WRITE && err != SSL_ERROR_WANT_READ) {
            if (LOGS_ENABLED) DEBUG_E("connection(%p) WS upgrade request write failed: %d", this, err);
            closeSocket(1, -1);
            return;
        }
    }
...
bool ConnectionSocket::processWebSocketUpgradeResponse() {
    uint8_t temp[4096];
    while (true) {
        int r = SSL_read(ssl, temp, sizeof(temp));
        if (r > 0) {
            workerInputBuffer.insert(workerInputBuffer.end(), temp, temp + r);
        } else {
            int err = SSL_get_error(ssl, r);
            if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
                break;
            }
            if (LOGS_ENABLED) DEBUG_E("connection(%p) SSL_read error during upgrade: %d", this, err);
            closeSocket(1, -1);
            return false;
        }
    }

    std::string dataStr(workerInputBuffer.begin(), workerInputBuffer.end());
    size_t headerEnd = dataStr.find("\r\n\r\n");
    if (headerEnd == std::string::npos) {
        if (workerInputBuffer.size() > 8192) {
            if (LOGS_ENABLED) DEBUG_E("connection(%p) WS upgrade response too large", this);
            closeSocket(1, -1);
            return false;
        }
        return false;
    ...
    if (headers.find(" 101 ") == std::string::npos && headers.find(" 101\r\n") == std::string::npos) {
        if (LOGS_ENABLED) DEBUG_E("connection(%p) invalid WS upgrade response: %s", this, headers.substr(0, std::min((size_t) 100, headers.size())).c_str());
        closeSocket(1, -1);
        return false;
    }
    workerInputBuffer.erase(workerInputBuffer.begin(), workerInputBuffer.begin() + headerEnd + 4);
    if (LOGS_ENABLED) DEBUG_D("connection(%p) Worker WS upgrade succeeded!", this);
    proxyAuthState = 23;
    if (!onConnectedSent) {
        lastEventTime = ConnectionsManager::getInstance(instanceNum).getCurrentTimeMonotonicMillis();
        onConnected();
    ...
    adjustWriteOp();
    if (!workerInputBuffer.empty()) {
        processWebSocketIncomingData();
    }
    return true;
}

void ConnectionSocket::writeWebSocketBinaryFrame(const uint8_t *data, size_t len) {
    if (ssl == nullptr || proxyAuthState != 23) {
        return;
    }
    ...
    for (size_t i = 0; i < len; ++i) {
        frame[payloadStart + i] = data[i] ^ mask[i % 4];
    }
    size_t totalSent = 0;
    while (totalSent < frame.size()) {
        int written = SSL_write(ssl, frame.data() + totalSent, (int) (frame.size() - totalSent));
        if (written > 0) {
            totalSent += written;
        } else {
            int err = SSL_get_error(ssl, written);
            ...
            if (LOGS_ENABLED) DEBUG_E("connection(%p) Worker SSL_write frame error: %d", this, err);
            closeSocket(1, -1);
            return;
        }
    }
...
void ConnectionSocket::sendWebSocketPong(const uint8_t *data, size_t len) {
    if (ssl == nullptr || proxyAuthState != 23) {
        return;
    }
    std::vector<uint8_t> frame;
    frame.push_back(0x8A);
    frame.push_back(static_cast<uint8_t>(0x80 | (len & 0x7F)));
    uint8_t mask[4];
    RAND_bytes(mask, 4);
    frame.insert(frame.end(), mask, mask + 4);
    for (size_t i = 0; i < len; ++i) {
...
bool ConnectionSocket::processWebSocketIncomingData() {
    uint8_t temp[4096];
    while (true) {
        int r = SSL_read(ssl, temp, sizeof(temp));
        if (r > 0) {
            ...
            if (err == SSL_ERROR_ZERO_RETURN) {
                if (LOGS_ENABLED) DEBUG_D("connection(%p) Worker SSL closed by peer", this);
            } else {
                if (LOGS_ENABLED) DEBUG_E("connection(%p) Worker SSL_read error: %d", this, err);
            }
            closeSocket(1, -1);
            return false;
        }
    ...
    size_t offset = 0;
    while (workerInputBuffer.size() - offset >= 2) {
        const uint8_t *data = workerInputBuffer.data() + offset;
        uint8_t b0 = data[0];
        uint8_t b1 = data[1];
        ...
        if (payloadLen == 126) {
            if (workerInputBuffer.size() - offset < 4) {
                break;
            }
            payloadLen = (static_cast<uint64_t>(data[2]) << 8) | data[3];
            headerLen = 4;
        } else if (payloadLen == 127) {
            if (workerInputBuffer.size() - offset < 10) {
                break;
            }
            payloadLen = 0;
            for (int i = 0; i < 8; ++i) {
        ...
        if (masked) {
            if (workerInputBuffer.size() - offset < headerLen + 4) {
                break;
            }
            maskKey = data + headerLen;
            headerLen += 4;
        }

        if (workerInputBuffer.size() - offset < headerLen + payloadLen) {
            break;
        }
        ...
        if (masked) {
            unmasked.resize(payloadLen);
            for (size_t i = 0; i < payloadLen; ++i) {
                unmasked[i] = payloadData[i] ^ maskKey[i % 4];
            }
            payloadData = unmasked.data();
        }
        ...
        if (opcode == 0x02 || opcode == 0x00) {
            if (payloadLen > 0) {
                ...
                buffer->clear();
                buffer->limit(static_cast<uint32_t>(payloadLen));
                buffer->writeBytes(const_cast<uint8_t*>(payloadData), static_cast<uint32_t>(payloadLen));
                buffer->rewind();
                if (ConnectionsManager::getInstance(instanceNum).delegate != nullptr) {
                    ConnectionsManager::getInstance(instanceNum).delegate->onBytesReceived((int32_t) payloadLen, currentNetworkType, instanceNum);
                }
                onReceivedData(buffer);
            }
        } else if (opcode == 0x09) {
            sendWebSocketPong(payloadData, payloadLen);
        } else if (opcode == 0x08) {
            if (LOGS_ENABLED) DEBUG_D("connection(%p) Worker WS received Close frame", this);
            closeSocket(1, -1);
            return false;
        }
    }
    ...
    if (offset > 0) {
        workerInputBuffer.erase(workerInputBuffer.begin(), workerInputBuffer.begin() + offset);
    }
    return true;
}
...
void ConnectionSocket::openConnection(std::string address, uint16_t port, std::string secret, bool ipv6, int32_t networkType) {
    ...
    int32_t proxyType = overrideProxyType;
    if (proxyAddress->empty()) {
        ...
        proxySecret = &ConnectionsManager::getInstance(instanceNum).proxySecret;
        proxyType = ConnectionsManager::getInstance(instanceNum).proxyType;
    }

    isWorker = (proxyType == 2 || proxyType == 3);
    if (!isWorker && !proxySecret->empty() && proxySecret->rfind("worker:", 0) == 0) {
        isWorker = true;
    }

    if (!proxyAddress->empty()) {
        if (LOGS_ENABLED) DEBUG_D("connection(%p) connecting via proxy %s:%d secret[%d] isWorker=%d", this, proxyAddress->c_str(), proxyPort, (int) proxySecret->size(), (int) isWorker);
        ...
        std::string hostToCheck = *proxyAddress;
        if (isWorker) {
            workerHost = *proxyAddress;
            if (workerHost.rfind("https://", 0) == 0) {
                workerHost = workerHost.substr(8);
            } else if (workerHost.rfind("http://", 0) == 0) {
                workerHost = workerHost.substr(7);
            }
            size_t slashPos = workerHost.find('/');
        ...
        if (inet_pton(AF_INET, hostToCheck.c_str(), &socketAddress.sin_addr.s_addr) != 1) {
            ...
            if (LOGS_ENABLED) DEBUG_D("connection(%p) not ipv4 address %s", this, hostToCheck.c_str());
        } else {
            ipv6 = false;
            continueCheckAddress = false;
        }
        if (continueCheckAddress) {
            if (inet_pton(AF_INET6, hostToCheck.c_str(), &socketAddress6.sin6_addr.s6_addr) != 1) {
                continueCheckAddress = true;
                if (LOGS_ENABLED) DEBUG_D("connection(%p) not ipv6 address %s", this, hostToCheck.c_str());
            } else {
                ipv6 = true;
                continueCheckAddress = false;
            }
            if (continueCheckAddress) {
                ...
                waitingForHostResolve = hostToCheck;
                ConnectionsManager::getInstance(instanceNum).delegate->getHostByName(hostToCheck, instanceNum, this);
                return;
#else
                struct hostent *he;
                if ((he = gethostbyname(hostToCheck.c_str())) == nullptr) {
                    if (LOGS_ENABLED) DEBUG_E("connection(%p) can't resolve host %s address", this, hostToCheck.c_str());
                    closeSocket(1, -1);
                    return;
                }
                ...
                if (addr_list[0] != nullptr) {
                    ...
                    if (LOGS_ENABLED) DEBUG_D("connection(%p) resolved host %s address %x", this, hostToCheck.c_str(), addr_list[0]->s_addr);
                    ipv6 = false;
                } else {
                    if (LOGS_ENABLED) DEBUG_E("connection(%p) can't resolve host %s address", this, hostToCheck.c_str());
                    closeSocket(1, -1);
    ...
    if (events & EPOLLIN) {
        ...
        if (checkSocketError(&error) != 0) {
            ...
            if (proxyAuthState == 20) {
                if (ssl == nullptr) {
                    SSL_CTX *ctx = getWorkerSslContext();
                    if (ctx == nullptr) {
                        closeSocket(1, -1);
                        return;
                    }
                    ...
                    SSL_set_tlsext_host_name(ssl, workerHost.c_str());
                    SSL_set_verify(ssl, SSL_VERIFY_NONE, nullptr);
                }
                int ret = SSL_connect(ssl);
                if (ret == 1) {
                    if (LOGS_ENABLED) DEBUG_D("connection(%p) Worker TLS connected", this);
                    sendWebSocketUpgradeRequest();
                    adjustWriteOp();
                } else {
                    int err = SSL_get_error(ssl, ret);
                    if (err != SSL_ERROR_WANT_READ && err != SSL_ERROR_WANT_WRITE) {
                        if (LOGS_ENABLED) DEBUG_E("connection(%p) Worker TLS connect error: %d", this, err);
                        closeSocket(1, -1);
                        return;
                    }
                }
            } else if (proxyAuthState == 22) {
                if (!processWebSocketUpgradeResponse()) {
                    return;
                }
            } else if (proxyAuthState == 23) {
                if (!processWebSocketIncomingData()) {
                    return;
                }
            } else {
                ssize_t readCount;
        ...
        if (checkSocketError(&error) != 0) {
            ...
            if (proxyAuthState >= 20) {
                if (proxyAuthState == 20) {
                    ...
                    if (remaining) {
                        writeWebSocketBinaryFrame(buffer->bytes(), remaining);
                        if (ConnectionsManager::getInstance(instanceNum).delegate != nullptr) {
...
void ConnectionSocket::adjustWriteOp() {
    ...
    if (((proxyAuthState == 0 || proxyAuthState == 23) && (outgoingByteStream->hasData() || !onConnectedSent)) ||
        proxyAuthState == 1 || proxyAuthState == 3 || proxyAuthState == 5 || proxyAuthState == 10 ||
        proxyAuthState == 20 || proxyAuthState == 21) {
...
void ConnectionSocket::setOverrideProxy(std::string address, uint16_t port, std::string username, std::string password, std::string secret, int32_t type) {
